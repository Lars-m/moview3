...
public class MovieResourceTest {
    ...
    public static void setUpClass() {
        ...
        //We are using the database on the virtual Vagrant image, so username password are the same for all dev-database
    ...
    public void testFindByTitleNotFound() {
        given().get("/movie/title/xxxxxxxxxx")
                .then()
                .assertThat()
                .statusCode(404)
...
//        given().log().all().get("/movie/" + m2.getId())
//                .then()
//                .log().body();
        given().get("/movie/" + m2.getId())
                .then()
                .assertThat()
                .statusCode(200)
                //.body("id", equalTo((m2.getId().intValue())));
...
//        movies = given().get("/movie/all").then().extract().body().jsonPath().getList(".", MovieDTO.class
//        );
//        boolean foundInFirst = Arrays.asList(movies.get(0).getActors()).contains("Brad Pitt");
//        boolean foundInSecond = Arrays.asList(movies.get(1).getActors()).contains("Brad Pitt");
//        boolean foundInThird= Arrays.asList(movies.get(2).getActors()).contains("Brad Pitt");
//        assertThat("One of the three movies should contain Brad Pitt", (foundInFirst || foundInSecond ||foundInThird));
//    }
